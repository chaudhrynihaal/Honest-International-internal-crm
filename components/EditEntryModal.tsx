...
interface EditEntryModalProps {
  entry: CreditLine & { quantity: number; unit: string; ratePerKg: number };
  onClose: () => void;
  onSuccess: () => void;
}
...
export function EditEntryModal({ entry, onClose, onSuccess }: EditEntryModalProps) {
  const [ratePerKg, setRatePerKg] = useState(String(entry.ratePerKg));
  const [note, setNote] = useState(entry.note ?? "");
  const [date, setDate] = useState(() => toDateInput(new Date(entry.date)));
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="card relative z-10 max-h-[90vh] w-full max-w-sm overflow-y-auto p-6">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-foreground">Edit Bill</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <p className="mt-1 text-sm text-foreground/60">
          ...
          {entry.yarnType ? ` · ${entry.yarnType}` : ""}
        </p>

        <form onSubmit={handleSubmit} className="mt-5 flex flex-col gap-4">
          <div>
            <label className="mb-1 block text-xs font-medium text-foreground/60">Rate (per kg)</label>
            <input
              type="number"
              min="0"
              step="0.01"
